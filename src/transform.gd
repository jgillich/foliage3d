...
func _generate(input: Array[Foliage3DPoint]) -> Array:
	...
	if rotation_min != Vector3.ZERO or rotation_max != Vector3.ZERO or rotation_absolute:
		for i in range(result.size()):
			...
			euler.x += deg_to_rad(rng.randf_range(rotation_min.x, rotation_max.x))
			euler.y += deg_to_rad(rng.randf_range(rotation_min.y, rotation_max.y))
			euler.z += deg_to_rad(rng.randf_range(rotation_min.z, rotation_max.z))
